...
<ResponseField name="--net" type="string" default="none">
  ...
  - `filtered` — Bridge network with an HTTP/HTTPS filtering proxy. Container gets `NetworkMode: "bridge"` with `HTTP_PROXY`/`HTTPS_PROXY` env vars pointing to the internal `proxy.mjs` process. Kernel-level iptables rules enforce that the sandbox user can only reach the proxy, preventing bypass via raw sockets.
</ResponseField>
