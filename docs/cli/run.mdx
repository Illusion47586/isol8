...
og:title: "isol8 run - CLI Reference"
og:description: "Complete reference for the 'isol8 run' command. Execute untrusted code, manage containers, and configure security options."
---
...
<ResponseField name="--install" type="string">
  Install a package before execution using the runtime's package manager. Repeatable — use `--install pkg1 --install pkg2` for multiple packages. Package names are validated to prevent command injection (allowed: alphanumeric, `-`, `_`, `.`, `/`, `@`, `=`).
</ResponseField>
...
<ResponseField name="--net" type="string" default="none">
  ...
  - `filtered` — Bridge network with an HTTP/HTTPS filtering proxy. Container gets `NetworkMode: "bridge"` with `HTTP_PROXY`/`HTTPS_PROXY` env vars pointing to the internal proxy process (`proxy.sh`). Kernel-level iptables rules enforce that the sandbox user can only reach the proxy, preventing bypass via raw sockets.
</ResponseField>
...
<ResponseField name="--log-network" type="boolean" default="false">
  Enable network request logging. When enabled with `--net filtered`, all HTTP/HTTPS requests made through the proxy are logged with timestamp, method, host, path, action (ALLOW/BLOCK), and duration. Logs are printed to stderr after execution completes. Requires `--net filtered` — silently no-ops for other network modes.
</ResponseField>
...
<ResponseField name="--seccomp" type="string" default="safety">
  Seccomp profile to use. Defaults to `safety` which blocks dangerous system calls (blacklist).
</ResponseField>
...
<ResponseField name="--git-clone" type="string">
  Clone a Git repository into the sandbox before execution. The URL can be HTTPS or SSH.
</ResponseField>

<ResponseField name="--git-clone-path" type="string">
  Path under `/sandbox` to clone into. Defaults to the repo name if omitted.
</ResponseField>
...
<ResponseField name="--git-commit" type="string">
  Commit changes after execution. By default, the CLI stages all changes before committing.
</ResponseField>

<ResponseField name="--git-commit-author-name" type="string">
  Author name to use for the commit.
</ResponseField>

<ResponseField name="--git-commit-author-email" type="string">
  Author email to use for the commit.
</ResponseField>

<ResponseField name="--git-push" type="boolean" default="false">
  Push committed changes after execution.
</ResponseField>

<ResponseField name="--git-push-branch" type="string">
  Branch to push (required when `--git-push` is provided).
</ResponseField>
...
isol8 run script.py --net filtered --log-network
```

The `--log-network` flag outputs JSON log entries to stderr:
