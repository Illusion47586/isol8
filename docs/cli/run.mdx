...
When `--url`, `--github`, or `--gist` is provided, source is fetched remotely and file/eval input is not allowed in the same request.
...
<ResponseField name="--url" type="string">
  Fetch source code from a URL. Requires `remoteCode.enabled=true` in config. Use `--hash` for integrity verification.
</ResponseField>

<ResponseField name="--github" type="string">
  GitHub shorthand in `owner/repo/ref/path/to/file` format. Expanded to `raw.githubusercontent.com`.
</ResponseField>

<ResponseField name="--gist" type="string">
  Gist shorthand in `gistId/file.ext` format. Expanded to `gist.githubusercontent.com`.
</ResponseField>

<ResponseField name="--hash" type="string">
  Expected SHA-256 hash for fetched remote code. Execution fails if the hash mismatches.
</ResponseField>

<ResponseField name="--allow-insecure-code-url" type="boolean" default="false">
  Allow insecure `http://` code URLs for this request.
</ResponseField>
...
# GitHub shorthand + hash verification
