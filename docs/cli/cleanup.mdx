...
og:title: "isol8 cleanup - CLI Reference"
og:description: "Manage and remove orphaned isol8 containers from your Docker environment. Essential for maintenance and freeing disk space."
---
...
- **Clean up after server crash/stop** — If `isol8 serve` stops unexpectedly, containers remain running. Use cleanup to remove them
...
You can manually configure cron to run cleanup periodically. isol8 does not automatically install or manage cron jobs — you need to set this up yourself.
...
- [`isol8 run`](/cli/run) — Execute code in isol8 containers
- [`isol8 config`](/cli/config) — View cleanup configuration (auto-prune, max age)
- [`isol8 serve`](/cli/serve) — Start server with automatic stale session cleanup
