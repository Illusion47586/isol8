...
og:title: "isol8 serve - CLI Reference"
og:description: "Start the isol8 HTTP server for remote code execution. Expose a secure API for running untrusted code."
---
...
<ResponseField name="--debug" type="boolean" default="false">
  Enable debug logging for internal server operations. When set, the server prints detailed logs about incoming requests, session management, engine lifecycle, semaphore acquisition, and auto-pruning. These logs use the `[DEBUG]` prefix and are suppressed by default. This flag is forwarded to the standalone server binary when running under Node.js.
</ResponseField>
...
The `isol8 serve` command uses a pre-compiled standalone binary that embeds the Bun runtime. This means you do **not** need Bun installed to run the server — only Docker is required.
...
4. The CLI spawns the binary as a child process, forwarding the `--port`, `--key`, and `--debug` arguments
...
# Start with debug logging to see request/session details
