...
|:--|:--|:--|:--|:--|
| `poolStrategy` | `--pool-strategy` | not in `isol8.config.json` | `options.poolStrategy` | `new DockerIsol8({ poolStrategy })` |
| `poolSize` | `--pool-size` | not in `isol8.config.json` | `options.poolSize` | `new DockerIsol8({ poolSize })` |
