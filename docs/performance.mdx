...
|:--|:--|:--|:--|:--|
| `poolStrategy` | `--pool-strategy` | `poolStrategy` (serve defaults) | `options.poolStrategy` | `new DockerIsol8({ poolStrategy })` |
| `poolSize` | `--pool-size` | `poolSize` (serve defaults) | `options.poolSize` | `new DockerIsol8({ poolSize })` |
...
<Note>
  Config-level `poolStrategy` and `poolSize` are server defaults (`isol8 serve`). CLI flags and API request `options` still override them per execution.
</Note>
