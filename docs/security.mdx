...
| Seccomp Profile | Strict | `security.seccomp` |
| Execution timeout | 30 seconds | `--timeout` / `timeoutMs` |

## Seccomp Profile

isol8 applies a "strict" seccomp profile by default to minimize the kernel attack surface. This profile blocks:
...
The profile allows standard application behavior but restricts privileged administrative actions typically not needed for code execution tasks.
