...
- **URL Allowlist/Blocklist**: Git clone URLs are checked against allowlists and blocklists (`gitSecurity.allowedHosts`, `gitSecurity.blockedPatterns`) to prevent SSRF.
- **Private IP Protection**: Private/internal IPs are blocked by default. Enable `gitSecurity.allowPrivateIPs` only for trusted internal Git servers.
- **Credential Masking**: Environment variables listed in `gitSecurity.credentialEnvVars` are masked from Git command output.
