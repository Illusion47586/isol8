...
Controls Git URL validation and credential masking for the Git operations API.

<ResponseField name="gitSecurity.allowedHosts" type="string[]" default="['github.com','gitlab.com','bitbucket.org',...]">
  Allowed Git hosts. If set and non-empty, only matching hosts are permitted (subdomains allowed). Default includes major public Git providers.
</ResponseField>

<ResponseField name="gitSecurity.blockedPatterns" type="string[]" default="[]">
  Regex patterns that always block a Git URL (SSRF protection). These are checked before the allowlist.
</ResponseField>

<ResponseField name="gitSecurity.allowPrivateIPs" type="boolean" default="false">
  Whether to allow private/internal IPs in Git URLs. Defaults to false for SSRF protection.
</ResponseField>

<ResponseField name="gitSecurity.credentialEnvVars" type="string[]" default="['GIT_TOKEN','GITHUB_TOKEN','GITLAB_TOKEN','BITBUCKET_TOKEN']">
  Environment variable names that may contain Git credentials. Values are masked in output logs.
</ResponseField>
