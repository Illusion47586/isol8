...
The server runs as a standalone compiled binary that embeds the Bun runtime — no Bun installation is required. The first time you run `isol8 serve`, the CLI downloads the binary for your platform from GitHub Releases and caches it at `~/.isol8/bin/isol8-server`. Use `--update` to force a re-download.
