...
<Tabs>
  <Tab title="CLI">
    Use the `--debug` flag to see internal engine logs.
  ...
  <Tab title="Library">
    Set `debug: true` in the constructor.

    ```typescript
    const isol8 = new DockerIsol8({ debug: true });
    ```
  </Tab>
  <Tab title="Server (API)">
    Start the server with `--debug` to log all request details.
