...
| Mode | Security | Description | Use Case |
|:-----|:---------|:------------|:---------|
| **`none`** | Highest | **Default**. No network interface. Blocks all traffic. | Most code execution tasks. |
| **`filtered`** | High | Bridge network with HTTP/HTTPS proxy + whitelist/blacklist. | API calls, scraping specific sites. |
| **`host`** | Low | Full host network access. | Trusted code needing unrestricted access. |
