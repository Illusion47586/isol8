...
Configurable via `--pool-strategy`.

| Strategy | Description | Behavior |
|:---|:---|:---|
| **`fast`** | **Default**. Dual-pool system. | Containers are returned to a "dirty" pool. A background task cleans them every 5s. Acquire is instant (O(1)). |
| **`secure`** | Synchronous cleanup. | Cleanup happens *inside* `acquire()`. Guarantees clean state but increases latency on every request. |
