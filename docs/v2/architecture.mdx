...
1. **Acquire**: Get a warm container from the pool (or create one if empty).
2. **Inject**: Write code to `/sandbox` via `docker exec`.
3. **Execute**: Run the code with timeout enforcement.
4. **Collect**: Capture stdout/stderr.
5. **Release**: Kill user processes (`pkill -9 -u sandbox`), wipe `/sandbox`, and return to pool.
...
- **Pool Size**: Configurable (default: 2 per image).
- **Lifecycle**:
