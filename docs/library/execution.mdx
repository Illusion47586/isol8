...
og:title: "Executing Code - isol8 Library"
og:description: "Learn how to execute code with the isol8 TypeScript library. Pass inputs, set timeouts, and handle structured results."
---
...
const result = await isol8.execute({
  ...
  git: {                             // Optional Git operations
    ...
    commit: { message: "feat: update", repoPath: "repo", all: true },
...
<ResponseField name="git" type="GitOperations">
  Git operations to run before and after execution. Operations are executed in order: clone → checkout → pull → execute → commit → push.
</ResponseField>

<ResponseField name="metadata" type="Record<string, string>">
  Additional metadata to include in audit logs. Useful for passing context like `userId`, `tenantId`, `requestId`, etc. Only used when audit logging is enabled.
</ResponseField>
...
Git URL safety rules are controlled by `gitSecurity` in `isol8.config.json` or `Isol8Options`. By default, only trusted public hosts are allowed and private IPs are blocked for SSRF protection.

### Audit Metadata

When audit logging is enabled, you can include metadata with each execution request. This metadata is recorded in the audit log and is useful for tracking user context, request IDs, or tenant information.
...
The metadata will appear in the audit log entry under the `metadata` field.
