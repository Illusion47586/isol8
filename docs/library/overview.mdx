...
og:title: "TypeScript Library Overview - isol8"
og:description: "Start using isol8 as a TypeScript/JavaScript library. Execute code locally with Docker or connect to a remote server."
---
...
<ResponseField name="poolStrategy" type="'fast' | 'secure'" default="fast">
  Container pool strategy for ephemeral mode.

  - **`fast`** (default): Uses a dual-pool system with clean and dirty pools. Instant acquire from clean pool, background cleanup every 5 seconds, immediate cleanup on acquire if clean pool is empty.
  - **`secure`**: Single pool, cleanup happens synchronously in acquire(). Slower but ensures container is always clean before use.
...
<ResponseField name="poolSize" type="number | { clean: number; dirty: number }" default="{ clean: 1, dirty: 1 }">
  ...
  - For `poolStrategy: "secure"`: Number of warm containers to maintain.
  - For `poolStrategy: "fast"`: Object with `clean` (ready containers) and `dirty` (being cleaned) counts.
</ResponseField>
