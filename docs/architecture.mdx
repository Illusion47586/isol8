...
- On release, the pool runs `rm -rf /sandbox/*` to clean state before returning the container
...
| Cleanup on release | `rm -rf /sandbox/*` | Executed via `docker exec` |
...
3. **`release(container, image)`** -- Wipes `/sandbox`, returns the container to the pool. If the pool is already at capacity, the container is destroyed instead.
...
HTTP/HTTPS traffic is routed through a filtering proxy (`proxy.mjs`) running inside the container. All other protocols are blocked.
...
sequenceDiagram
    ...
    Code->>Proxy: HTTP request to api.example.com
    ...
    Code->>Proxy: HTTPS CONNECT to api.example.com:443
    alt Allowed
        Proxy->>Internet: TCP tunnel (CONNECT)
        Note over Proxy,Internet: TLS passthrough, proxy cannot inspect
        Internet-->>Code: Encrypted data
    else Blocked
        Proxy-->>Code: 403 Forbidden
    end
```
...
<Note>
  The Deno stage does **not** extend `base`. It uses `denoland/deno:alpine` as its base image and re-applies the base setup (tini, proxy, workdir, entrypoint) manually. This is because Deno's official Alpine image has its own specific setup requirements.
</Note>
...
| Sandbox tmpfs | `/sandbox` (default 64MB) | Writable workspace for code and files |
| Tmp tmpfs | `/tmp` (default 32MB) | Temporary files |
