...
og:title: "Architecture & Design - isol8"
og:description: "Explore the internal architecture of isol8. Learn how the container pool, execution pipeline, and concurrency model work under the hood."
---
...
HTTP/HTTPS traffic is routed through a filtering proxy (`proxy.sh` + `proxy-handler.sh`) running inside the container. Kernel-level iptables rules enforce that the `sandbox` user can **only** reach the proxy — all other outbound traffic is dropped.

```mermaid
sequenceDiagram
    participant Code as User Code
    participant IPT as iptables (kernel)
    participant Proxy as proxy.sh (127.0.0.1:8118)
...
| Network proxy | `proxy.sh` + `proxy-handler.sh` copied to `/usr/local/bin/` |
