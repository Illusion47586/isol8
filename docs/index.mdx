---
title: "Introduction"
description: "Secure code execution engine for AI agents. Run untrusted code inside Docker containers with strict resource limits, network controls, and output sanitization."
og:title: "isol8 Documentation - Secure Code Execution Engine"
og:description: "Secure code execution engine for AI agents. Learn how to safely execute untrusted code in Docker containers with the CLI, TypeScript library, and HTTP server."
---
...
<CardGroup cols={2}>
  ...
  <Card title="Sub-150ms Latency" icon="gauge-high">
    The warm container pool keeps pre-started containers ready for immediate use, reducing execution latency from ~250-300ms (cold start) to ~125-145ms (warm). Containers are cleaned and recycled after each run.
  </Card>
