...
**`filtered`** gives the container bridge networking with an HTTP/HTTPS proxy that filters requests by hostname. Use `--allow` and `--deny` regex patterns to control which hosts are reachable. Non-HTTP protocols (raw TCP, UDP, WebSocket) are blocked.
