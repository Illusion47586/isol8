...
Enable detailed logging of all HTTP/HTTPS requests made through the filtering proxy in `filtered` network mode:
...
const result = await engine.execute({
  code: "import urllib.request; urllib.request.urlopen('https://api.example.com')",
  runtime: "python",
});

console.log(result.networkLogs);
```

### Log Format

Each network request produces a JSON entry:
...
For HTTPS CONNECT tunnels (which don't have a path):
...
| `path` | string \| null | Request path for HTTP, null for HTTPS CONNECT |
| `action` | string | `ALLOW` or `BLOCK` |
| `durationMs` | number | Time taken to handle the request in milliseconds |

### Use Cases

- **API Usage Tracking**: Monitor which external APIs your code is accessing
- **Security Auditing**: Review all network requests for compliance
- **Debugging**: Trace network issues in executed code
- **Billing**: Track API call counts by host

### In Audit Logs

Network logs are also included in audit log entries when enabled:
