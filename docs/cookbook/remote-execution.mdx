...
og:title: "Remote Execution - isol8 Cookbook"
og:description: "Scale your execution engine. Deploy isol8 as a service and connect from anywhere."
---
...
```bash
# Start the server (downloads standalone binary on first use)
isol8 serve --port 3000 --key my-secret-api-key

# Force re-download the server binary
isol8 serve --port 3000 --key my-secret-api-key --update
```

Or programmatically (requires Bun runtime):
