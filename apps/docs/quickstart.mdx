...
<Callout type="warning">
  **Deprecation Notice**: The `isol8` package is deprecated. Please use:
  - **CLI**: `@isol8/cli` 
  - **Library**: `@isol8/core`
</Callout>
...
<Steps>
  <Step title="Install and verify">
    ...
    <CodeGroup>
    ```bash Command
    @isol8/cli --version && docker info >/dev/null
    ```
  ...
  <Step title="Run code">
    ...
    <CodeGroup>
    ```bash Command
    @isol8/cli run -e "print('hello from sandbox')" --runtime python
    ```
