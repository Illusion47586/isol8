...
Returned when the server's connection queue is full. All execution slots are occupied and the queue has reached its `queue.maxSize` limit.
...
<Tip>
  If you see frequent 429 or 408 responses, increase `maxConcurrent`, `queue.maxSize`, or `queue.timeoutMs` in your [configuration](/configuration).
</Tip>
