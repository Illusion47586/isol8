...
<Tabs>
  ...
  <Tab title="With DB-backed auth">
    ```bash
    # SQLite (default path ~/.isol8/auth.db)
    isol8 serve --port 3000 --key "$ISOL8_API_KEY" --auth-db
...
- **No value or file path** (e.g. `./isol8-auth.db`) → SQLite
...
- connection queueing with configurable size limits and timeouts (`queue.maxSize`, `queue.timeoutMs`)
- backpressure: `429` when queue is full, `408` when queued request times out
- queue monitoring via `GET /queue/status`
...
flowchart TD
  ...
  B --> C["Auth middleware"]
  C --> D["Global semaphore + queue"]
  D --> E["DockerIsol8 engine"]
  E --> F["Sandbox container(s)"]
  D -- "queue full" --> G["429 Too Many Requests"]
  D -- "queue timeout" --> H["408 Request Timeout"]
