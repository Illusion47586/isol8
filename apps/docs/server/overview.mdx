...
Pass `--auth-db <path>` to enable DB-backed API key management with SQLite. When enabled, the `--key` value becomes the master key with admin privileges, and additional keys can be managed via the `/auth/*` endpoints. See <a href="/server/routes">Server routes</a> for full endpoint reference.
...
Enable with `--auth-db <path>` to store API keys in SQLite. When enabled:
