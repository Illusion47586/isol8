...
<ResponseField name="--auth-db <path>" type="string">
  Enable DB-backed API key management with a SQLite database at `<path>`.
...
<Accordion title="What is the difference between `--key` and `--auth-db`?">
  `--key` sets a single static API key (or master key). `--auth-db` enables SQLite-backed key management where you can create, list, and revoke multiple keys via the `/auth/*` endpoints. Both can be used together — the `--key` becomes the master key when `--auth-db` is enabled.
</Accordion>
