...
<Callout type="warning">
  **Deprecation Notice**: The `isol8` CLI is deprecated. Please use `@isol8/cli`.
</Callout>
...
```bash
@isol8/cli serve --key <api-key> [options]
```
...
```bash
@isol8/cli config [options]
```

## `@isol8/cli cleanup`

```bash
@isol8/cli cleanup [options]
```
