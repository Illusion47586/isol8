...
When the server is started with `--auth-db <connection>`, API keys are stored in a database. Supported backends: SQLite (file path), PostgreSQL (`postgres://` or `postgresql://`), and MySQL (`mysql://`). The `--key` value becomes the **master key** with admin privileges:
...
For server-side key management, use the `createServer` function with `authDbPath` (supports SQLite file paths, `postgres://`, and `mysql://` connection strings):
