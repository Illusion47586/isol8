...
When the server is started with `--auth-db <path>`, API keys are stored in SQLite. The `--key` value becomes the **master key** with admin privileges:
...
For server-side key management, use the `createServer` function with `authDbPath`:
