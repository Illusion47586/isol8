...
## Concurrency and Queue Utilities

The `Semaphore` class and related queue types are exported from `@isol8/core` for advanced use cases.
...
import type { QueueOptions, SemaphoreStats } from "@isol8/core";
...
// Monitor state
const stats: SemaphoreStats = sem.stats;
// { current: 1, max: 3, available: 2, pending: 0 }
```

<ResponseField name="QueueFullError" type="class">
  Thrown when the semaphore's waiting queue is at capacity (`maxSize > 0` and queue is full). Properties: `queueSize`, `maxSize`.
</ResponseField>

<ResponseField name="QueueTimeoutError" type="class">
  Thrown when a queued `acquire()` call exceeds the configured timeout. Properties: `timeoutMs`, `waitedMs`.
</ResponseField>

<ResponseField name="QueueOptions" type="interface">
  Options for the semaphore queue: `maxSize` (default `0` = unlimited) and `timeoutMs` (default `0` = unlimited).
</ResponseField>

<ResponseField name="SemaphoreStats" type="interface">
  Snapshot of semaphore state: `current`, `max`, `available`, `pending`.
</ResponseField>
