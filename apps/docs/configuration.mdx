...
Connection queue settings for the server. Controls backpressure when all execution slots are busy.

<ResponseField name="queue.maxSize" type="number" default="0">
  Maximum number of requests that can wait in the queue. When the queue is full, new requests are rejected immediately with HTTP `429`. `0` means unlimited (no cap on queue size).
</ResponseField>

<ResponseField name="queue.timeoutMs" type="number" default="30000">
  Maximum time in milliseconds a request can wait in the queue before timing out. When exceeded, the request is rejected with HTTP `408`. `0` means unlimited (no timeout).
</ResponseField>
...
| `queue` | object | `{ maxSize: 0, timeoutMs: 30000 }` | Server connection queue / backpressure |
...
- **Requests rejected with 429**: queue is full — increase `queue.maxSize` or `maxConcurrent`.
- **Requests rejected with 408**: requests timing out in queue — increase `queue.timeoutMs` or `maxConcurrent`.
